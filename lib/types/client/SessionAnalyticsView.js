...
function TokenTrendChart({ history, now, rangeMs, t }) {
    ...
    return (_jsxs("div", { className: css.chartBox, children: [_jsxs("svg", { className: css.chart, viewBox: `0 0 ${W} ${H}`, role: "img", "aria-label": t('chart.tokens'), children: [_jsx("defs", { children: _jsxs("linearGradient", { id: "wsTrendFill", x1: "0", y1: "0", x2: "0", y2: "1", children: [_jsx("stop", { offset: "0%", className: css.chartAreaTop }), _jsx("stop", { offset: "100%", className: css.chartAreaBottom })] }) }), ticks.map(tk => (_jsxs("g", { children: [_jsx("line", { className: css.chartGrid, x1: PAD.left, x2: W - PAD.right, y1: tk.y, y2: tk.y }), _jsx("text", { className: css.chartAxis, x: PAD.left - 6, y: tk.y + 3, textAnchor: "end", children: formatTokens(tk.v) })] }, tk.f))), series.map((s, i) => {
                        const cx = xOf(i);
                        const outTop = yOf(s.total);
                        const inTop = yOf(s.input);
                        return (_jsxs("g", { children: [_jsx("rect", { className: css.chartBarIn, x: cx - barW / 2, y: inTop, width: barW, height: Math.max(1, baseline - inTop), rx: 2 }), _jsx("rect", { className: css.chartBarOut, x: cx - barW / 2, y: outTop, width: barW, height: Math.max(1, inTop - outTop), rx: 2 })] }, s.key));
                    }), areaPath !== '' && _jsx("path", { className: css.chartArea, d: areaPath }), _jsx("path", { className: css.chartLine, d: topPath }), series.map((s, i) => (_jsxs("g", { children: [_jsx("circle", { className: css.chartDot, cx: xOf(i), cy: yOf(s.total), r: 3.5 }), _jsx("text", { className: css.chartAxis, x: xOf(i), y: H - 8, textAnchor: "middle", children: dayLabel(s.key) })] }, s.key)))] }), _jsxs("div", { className: css.chartLegend, children: [_jsxs("span", { className: css.legendItem, children: [_jsx("span", { className: css.legendBarIn }), t('chart.input')] }), _jsxs("span", { className: css.legendItem, children: [_jsx("span", { className: css.legendBarOut }), t('chart.output')] }), _jsxs("span", { className: css.legendItem, children: [_jsx("span", { className: css.legendLine }), t('chart.total')] })] })] }));
}
